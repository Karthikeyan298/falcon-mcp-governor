...
class ApprovalService:
    ...
    def _execute_approved(self, row: dict, approval_id: int, decision_by: str) -> None:
        ...
        args_json = row['arguments']  # already JSON-serialised in the approvals table
        ...
        try:
            ...
            pass  # Leave as approved; gateway fallback handles retry
    ...
    def deny(self, approval_id: int, decision_by: str) -> dict:
        with self._database.connect() as conn:
            ...
            if row['status'] != 'pending':
                raise NotFoundError(f'Approval #{approval_id} is already {row["status"]}')
            repo.decide(approval_id, 'denied', decision_by, self._database.now_iso())
